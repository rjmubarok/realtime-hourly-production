...
@section('content')
    <div class="card-header bg-cyan">
        <div class="row">
            <div class="col-md-6">
                <h3 class="card-title text-bold mt-2">Defect Code List</h3>
            </div>
            <div class="col-md-6 text-right">
                <a href="{{route('defectSetup' )}}" class="btn btn-primary"> Create New </a>
            </div>
        </div>
    </div>
    <div class="card-body">
                ...
                <table id="example" class="table table-striped datatable table-bordered">
                    ...
                    @foreach($datas as $data)
                        <tr style="background-color:#f5eded;">
                            ...
                            <td>
                                <a href="{{ route('defect_edit', $data->id) }}" class="btn btn-sm btn-info p-1" title="Edit"><i class="fas
                                fa-edit"></i></a>
                                @if($data->status==0)
                                    <a href="{{ route('defectStatus', $data->id) }}" class="btn btn-sm btn-success p-1"
                                       title="Active"><span><i class="fas fa-check"></i></span></a>
                                @else
                                    <a href="{{ route('defectStatus', $data->id) }}" class="btn btn-sm btn-danger p-1"
